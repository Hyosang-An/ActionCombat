...
void UEnemyProjectileComponent::SpawnProjectile(FName ComponentName, TSubclassOf<AActor> ProjectileClass)
{
	...
	FTransform SpawnTransform = SpawnPointComp->GetComponentTransform();

	FVector PlayerLocation = GetWorld()->GetFirstPlayerController()->GetPawn()->GetActorLocation();

	FRotator SpawnRotator = UKismetMathLibrary::FindLookAtRotation(SpawnLocation, PlayerLocation);
	
	FActorSpawnParameters SpawnParams;
	SpawnParams.Owner = GetOwner();
	SpawnParams.Instigator = GetOwner<APawn>();
	GetWorld()->SpawnActor(ProjectileClass, &SpawnLocation, &SpawnRotator, SpawnParams);
}